...
const SpotifyLoginContainer = styled.div`
  padding: 20px;
  max-width: 500px;
  margin: 0 auto;
  position: relative;
...
const SpotifyLogin = ({ isAdmin }) => {
  ...
  const handleLogin = () => {
    if (spotifyLoginUrl) {
      ...
      // Try to get the URL again if it's not available
      dispatch(getSpotifyLoginUrl());
    }
  };
  ...
  return (
    <JukeboxContainer>
      <SpotifyLoginContainer>
        ...
        <SpotifyForm>
          <NeonText>
            {isAdmin ? 'Admin Spotify' : 'Guest Spotify'}
          </NeonText>
          
          <InstructionText>
            {isAdmin 
              ? 'Connect your Spotify account to manage the jukebox playlist and control playback.' 
              : 'Connect your Spotify account to add songs to the jukebox queue.'}
          </InstructionText>
          ...
          {isAdmin && (
            <InstructionText style={{ fontSize: '0.9rem', color: 'var(--color-gold)' }}>
              Note: As an administrator, you'll need a Spotify Premium account to control playback on Spotify Connect devices.
            </InstructionText>
          )}
          
          <BackButton 
            type="button" 
            onClick={() => window.location.href = isAdmin ? '/login' : '/guest'}
          >
            Back
          </BackButton>
        </SpotifyForm>
